...
for file_signal in files_signal_list:
    ...
    df_largest = df_signal.nlargest(10, columns = "est_counts")
    ...
    for row in df_largest.itertuples():
        ...
        ax[rows,col].bar(df_signal_plot["start"] + (df_signal_plot["length"] / 2),
                         height = df_signal_plot["est_counts"],
                         width = df_signal_plot["length"],
                         color = ["green" if count == row[7] else 
                                  "blue"
                                  for count in df_signal_plot["est_counts"]])
        
        ax[rows,col].set_ylabel("log2(signal/noise)")
        ax[rows,col].set_xlabel("positions")
        
        ax[rows,col].set_title(f"{chrom_names[row[1]]}\n"
                               f"{row[7]} reads")
    ...
    path = ("/datas/nathan/vscode_nextflow/nextflow-nathan/results/analysis/figures/highest_peak_count/"
            + sample_name
            + "_10_highest_count_peaks_count.eps")
